<?php
	...
	$sql0 = "SELECT * FROM `" . $user . "` WHERE No = " . $user_id . "";
	...
	if (isset($_POST['accept']) && intval($_POST['accept'])) {
		# code...
		$sql2 = "UPDATE `" . $username . "` SET Status = 'Forwarded' WHERE No = " . $row0['ID'] . "";
		$result2 = $conn->query($sql2);

		$sql3 = "UPDATE `" . $user . "` SET Status = 'Forwarded' WHERE No = " . $user_id . "";
	...
	if (isset($_POST['reject']) && intval($_POST['reject'])) {
		...
		$sql3 = "UPDATE `" . $user . "` SET Status = 'Denied' WHERE No = " . $user_id . "";
		$result3 = $conn->query($sql3);
